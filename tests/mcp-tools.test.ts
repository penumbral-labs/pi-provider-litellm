...
describe("partial discovery diagnostics", () => {
  it("emits the generated diagnostic again after the incident clears", () => {
    ...
    const tools = ["mcp_a_one_1", "mcp_a_two_2", "mcp_a_three_3"];
    reportMcpPartialDiscovery(true, tools);
    reportMcpPartialDiscovery(false, tools);
    reportMcpPartialDiscovery(true, tools);
  ...
  it("reports again when the surviving tools change but their count does not", () => {
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);

    reportMcpPartialDiscovery(true, ["mcp_a_one_1"]);
    reportMcpPartialDiscovery(true, ["mcp_a_one_1"]);
    reportMcpPartialDiscovery(true, ["mcp_b_one_2"]);
...
describe("schema keyword validation positions", () => {
  ...
  it("classifies a non-schema additionalItems value as invalid-schema", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      jsonResponse(200, [
        {
          name: "invalid-additional-items",
          server_name: "srv",
          input_schema: { type: "object", properties: { x: { type: "array", additionalItems: 5 } } },
        },
      ]),
    );
    ...
    expect(report.dropped).toEqual([
      {
        ...
        tools: [expect.stringMatching(/^mcp_srv_invalid_additional_items_[a-f0-9]{10}$/)],
      },
    ]);
  });
...
describe("untrusted schema hazards", () => {
  ...
  const hazardCatalogs: Array<[string, unknown]> = [
    ...
    // Coverage includes hazards outside standard schema-position tables.
    [
  ...
  describe("negative controls: these must keep their supplied schema", () => {
    const clean: Array<[string, unknown]> = [
      ...
      [
        "a local ref to an additionalItems subschema",
        {
          type: "object",
          properties: {
            x: { type: "array", items: [{ type: "string" }], additionalItems: { type: "number" } },
            extra: { $ref: "#/properties/x/additionalItems" },
          },
        },
      ],
...
describe("loss accounting reconciles to the raw catalog", () => {
  ...
  it("does not echo a rejected entry's name when its server is missing", async () => {
    vi.resetModules();
    const { createMcpToolDefinitions: createDefinitionsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse(200, { tools: [{ name: "skSecret" }] }));

    await createDefinitionsRaw(auth);

    const diagnostics = stderr.mock.calls.map(([message]) => String(message)).join("");
    expect(diagnostics).toContain("entry_0");
    expect(diagnostics).not.toContain("skSecret");
  });
...
describe("incident identity covers full membership", () => {
  ...
  it("reports a body-cap breach again after a bodyless response on the same surface", async () => {
    vi.resetModules();
    const { discoverMcpTools: discoverToolsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    const fetchMock = vi.spyOn(globalThis, "fetch");
    ...
    // A bodyless response is within the cap too, so it clears the incident...
    fetchMock.mockImplementation(async () => new Response(null, { status: 200 }));
    await expect(discoverToolsRaw("https://litellm.example.com", "sk-test")).rejects.toThrow(/invalid JSON/);
...
describe("bounded diagnostic labels", () => {
  it("re-reports a different malformed entry at the same position but suppresses an unchanged one", async () => {
    ...
    const { createMcpToolDefinitions: createDefinitionsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    let badEntry = { name: "first-private-name" };
    vi.spyOn(globalThis, "fetch").mockImplementation(async () => jsonResponse(200, { tools: [badEntry] }));
    const auth = async () => ({ baseUrl: "https://litellm.example.com", apiKey: "sk-test" });
    const invalidLines = () =>
      stderr.mock.calls
        .map(([message]) => String(message))
        .filter((message) => message.includes("missing name or server identity"));

    await createDefinitionsRaw(auth);
    await createDefinitionsRaw(auth);
    expect(invalidLines()).toHaveLength(1);

    badEntry = { name: "second-private-name" };
    await createDefinitionsRaw(auth);
    await createDefinitionsRaw(auth);

    expect(invalidLines()).toHaveLength(2);
    expect(invalidLines()).toEqual([
      "LiteLLM MCP: dropped 1 MCP tool with a missing name or server identity: entry_0.\n",
      "LiteLLM MCP: dropped 1 MCP tool with a missing name or server identity: entry_0.\n",
    ]);
    expect(invalidLines().join("")).not.toContain("private-name");
  });

  it("re-reports a malformed entry when its positional label changes", async () => {
    vi.resetModules();
    const { createMcpToolDefinitions: createDefinitionsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    const badEntry = { name: "private-name" };
    let tools: unknown[] = [badEntry];
    vi.spyOn(globalThis, "fetch").mockImplementation(async () => jsonResponse(200, { tools }));
    const auth = async () => ({ baseUrl: "https://litellm.example.com", apiKey: "sk-test" });
    const invalidLines = () =>
      stderr.mock.calls
        .map(([message]) => String(message))
    ...
    await createDefinitionsRaw(auth);
    tools = [{ name: "ok", server_name: "srv" }, badEntry];
    await createDefinitionsRaw(auth);

    expect(invalidLines()).toEqual([
      "LiteLLM MCP: dropped 1 MCP tool with a missing name or server identity: entry_0.\n",
      "LiteLLM MCP: dropped 1 MCP tool with a missing name or server identity: entry_1.\n",
    ]);
    expect(invalidLines().join("")).not.toContain("private-name");
  });

  it("re-reports malformed entries that differ only after 64 KiB", async () => {
    vi.resetModules();
    const { createMcpToolDefinitions: createDefinitionsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    let badEntry = { name: "A".repeat(70_000), privateValue: 1 };
    vi.spyOn(globalThis, "fetch").mockImplementation(async () => jsonResponse(200, { tools: [badEntry] }));
    const auth = async () => ({ baseUrl: "https://litellm.example.com", apiKey: "sk-test" });
    const invalidLines = () =>
      stderr.mock.calls
    ...
    await createDefinitionsRaw(auth);
    badEntry = { name: "A".repeat(70_000), privateValue: 2 };
    await createDefinitionsRaw(auth);

    expect(invalidLines()).toHaveLength(2);
    expect(invalidLines().every((line) => line.includes("entry_0"))).toBe(true);
    expect(invalidLines().join("")).not.toContain("privateValue");
  });

  it("labels a normalization failure positionally instead of echoing a huge proxy-supplied name", async () => {
    vi.resetModules();
    const { createMcpToolDefinitions: createDefinitionsRaw } = await import("../src/mcp-tools.js");
    const stderr = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
...
describe("MCP normalization edge cases", () => {
