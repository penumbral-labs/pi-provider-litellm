...
describe("extension startup", () => {
  ...
  it("applies the LiteLLM request hook to configured provider aliases", async () => {
    const agentDir = await makeAgentDir();
    await writeFile(
      join(agentDir, "settings.json"),
      JSON.stringify({
    ...
    const result = await pi.handlers.get("before_provider_request")?.[0]?.(
      { payload: { model: "gpt-5.5", tools: [{}], reasoning_effort: "high" } },
      { model: { provider: "litellm-anthropic", id: "gpt-5.5", api: "openai-completions" } },
    );

    expect(result).toEqual({ model: "gpt-5.5", tools: [{}] });
  });
