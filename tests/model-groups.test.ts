...
import { type CatalogResolution, type CatalogResolver, reduceModelGroup, stableJson } from "../src/model-groups.js";
...
describe("reduceModelGroup", () => {
  ...
  it("withholds a route containing a chat-style and explicitly incompatible deployment", () => {
    const chat = row({ model_info: { id: "chat", mode: "chat" } });
    const embedding = row({
    ...
    expect(reduceModelGroup([chat, embedding], resolveCatalog)).toBeUndefined();
  });
  ...
  it("treats an unreadable mode as unknown rather than as evidence of a non-chat deployment", () => {
    ...
    // Genuinely non-chat: the mixed route is withheld rather than partially published.
  ...
  it("uses the smaller router and catalog limit before taking the safe group minimum", () => {
    const routerLargerThanCatalog = row({
      model_info: { id: "large-router", mode: "chat", max_input_tokens: 300_000, max_output_tokens: 80_000 },
    });
    expect(reduceModelGroup([routerLargerThanCatalog], resolveCatalog)).toMatchObject({
      contextWindow: 200_000,
      maxTokens: 64_000,
    });
  ...
  it("marks metadata complete only when every published field is resolved", () => {
    ...
    const unknownMode = row({ model_info: { mode: null } });
    expect(reduceModelGroup([unknownMode], resolveCatalog)).toMatchObject({ hasCompleteMetadata: true });
  ...
  it("rejects negative per-token prices as unresolved", () => {
    const result = reduceModelGroup(
      [
        row({
          model_info: { input_cost_per_token: -0.000004 },
          litellm_params: { model: "internal/unknown" },
        }),
      ],
      resolveCatalog,
    ...
    expect(result).toMatchObject({ hasCompleteMetadata: false, cost: { input: 0 } });
  });
  ...
  it("does not apply a catalog ladder to a fully operator-priced deployment", () => {
    const result = reduceModelGroup([row()], () => ({
      provider: "anthropic",
      reasoning: true,
      vision: true,
  ...
  it("substitutes a partial explicit price into every catalog tier", () => {
    const partial = row({
      model_info: {
        input_cost_per_token: 0.000004,
        output_cost_per_token: undefined,
    ...
    const result = reduceModelGroup([partial], () => ({
      provider: "anthropic",
      reasoning: true,
      vision: true,
      contextWindow: 200_000,
    ...
    expect(result?.cost.tiers).toEqual([
      { inputTokensAbove: 200_000, input: 4, output: 22.5, cacheRead: 0.6, cacheWrite: 7.5 },
      { inputTokensAbove: 400_000, input: 4, output: 30, cacheRead: 0.9, cacheWrite: 11.25 },
    ]);
  });

  it("builds a union-threshold upper envelope across deployment ladders", () => {
    ...
    const ladders = {
      a: [{ inputTokensAbove: 200_000, input: 6, output: 20, cacheRead: 0.6, cacheWrite: 7 }],
      b: [{ inputTokensAbove: 400_000, input: 8, output: 25, cacheRead: 0.8, cacheWrite: 9 }],
    };
    const result = reduceModelGroup(rows, (entry) => ({
      provider: "anthropic",
      reasoning: true,
      vision: true,
      contextWindow: 200_000,
      maxTokens: 64_000,
      cost: {
        input: entry.model_info?.id === "a" ? 3 : 4,
        output: entry.model_info?.id === "a" ? 15 : 18,
        cacheRead: entry.model_info?.id === "a" ? 0.3 : 0.4,
        cacheWrite: entry.model_info?.id === "a" ? 3.75 : 4.5,
        tiers: ladders[entry.model_info?.id as keyof typeof ladders],
      },
    }));

    expect(result?.cost.tiers).toEqual([
      { inputTokensAbove: 200_000, input: 6, output: 20, cacheRead: 0.6, cacheWrite: 7 },
      { inputTokensAbove: 400_000, input: 8, output: 25, cacheRead: 0.8, cacheWrite: 9 },
    ]);
  });
