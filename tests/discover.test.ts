...
describe("discoverModels via /model/info", () => {
  ...
  it("marks an ambiguous group incomplete despite complete router pricing and reports withheld authority", async () => {
    ...
    // Pricing is complete, but the limits and capabilities below are defaults the
    // reducer had to assume, so the model still carries the incomplete marker.
    expect(result.models[0]).toMatchObject({
  ...
  it("keeps baseline compatibility metadata for Responses-mode routes", async () => {
    // Compat is derived from the model id independently of transport. Passing API
    // into this decision would make a Responses-mode Moonshot route silently lose
    // these Kimi repairs, so the whole object is pinned exactly.
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
...
describe("discoverModels wildcard expansion via /v1/models", () => {
  ...
  it("uses a conservative tier envelope when matching wildcard parents differ", async () => {
    const broad = {
      model_name: "*",
      litellm_params: { model: "openai/gpt-5.4" },
    ...
    expect(resolveModelInfoCatalog(broad)?.cost?.tiers).not.toEqual(resolveModelInfoCatalog(narrow)?.cost?.tiers);
    mockEndpoints({
      "/model/info": () => jsonResponse(200, { data: [broad, narrow] }),
      "/v1/models": () => jsonResponse(200, { data: [{ id: "team/assistant" }] }),
    });

    const result = await discoverModels("https://litellm.example.com", "sk-test", { modelsDev: false });

    expect(result.models[0]?.cost.tiers).toBeDefined();
    expect(result.models[0]?.cost.tiers?.map((tier) => tier.inputTokensAbove)).toEqual(
      [
        ...new Set([
          ...(resolveModelInfoCatalog(broad)?.cost?.tiers ?? []).map((tier) => tier.inputTokensAbove),
          ...(resolveModelInfoCatalog(narrow)?.cost?.tiers ?? []).map((tier) => tier.inputTokensAbove),
        ]),
  ...
  it("closes levels when overlapping wildcards evidence incompatible Chat carriers", async () => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [
      ...
      "/v1/models": () => jsonResponse(200, { data: [{ id: "team/reasoner" }] }),
    });
...
describe("discoverModels response-mode models", () => {
  ...
  it("keeps /health response-mode model_info fallbacks on Responses", async () => {
    ...
    expect(result.source).toBe("health");
    expect(result.models).toHaveLength(1);
    // The per-deployment detail is the same /model/info row rich discovery reads; its
    // explicit Responses mode is not weaker evidence for having arrived via /health.
    expect(result.models[0]).toMatchObject({
      id: "openai/gpt-5.3-codex-openai",
      api: "openai-responses",
    });
  ...
  it("does not derive thinking controls from a health-only route name", async () => {
    ...
    expect(result.source).toBe("health");
    expect(result.models[0]).toMatchObject({ id: "openai/gpt-5.5", reasoning: false });
    expect(result.models[0]).not.toHaveProperty("thinkingLevelMap");
  });
  ...
  it("does not derive thinking controls from a health endpoint without deployment detail", async () => {
    // No `model_id`, so the route name is the only input and cannot produce a
    // reasoning selector or borrow catalog metadata.
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
...
describe("catalog provider candidates", () => {
  ...
  it.each([
    ...
    "fable-5",
    "opus-5",
  ])("resolves the Anthropic backend catalog id %s from the shared lookup rule", async (id) => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
  ...
  it.each(["claudia-x", "opusclip-2", "gpt-4o", "haiku"])("does not treat %s as an Anthropic alias", async (id) => {
    ...
    expect(result.models[0]?.name).toBe(`route-${id} (incomplete metadata)`);
...
describe("discoverModels fallback to /health", () => {
  ...
  it("withholds catalog metadata from health routes without correlated backend evidence", async () => {
    // `/health` route text is never authorized for catalog or later cache enrichment,
    // so both unknown and catalog-resolvable names carry the permanent marker rather
    // than the `/v1/models` sentinel.
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
      "/health": () =>
    ...
    expect(result.source).toBe("health");
    const [unresolved, catalogNamed] = result.models;
    ...
    expect(catalogNamed).toMatchObject({
