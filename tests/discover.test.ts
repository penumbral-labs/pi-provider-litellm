...
describe("normalizeBaseUrl", () => {
  it("rejects insecure non-loopback endpoints by default", () => {
    expect(() => normalizeBaseUrl("http://litellm.example.com")).toThrow(/HTTPS/);
  });

  it("allows explicitly configured insecure HTTP endpoints", () => {
...
describe("discoverModels via /model/info", () => {
  ...
  it("classifies codex model identities as OpenAI family", () => {
    expect(
      resolveModelInfoCatalog({
        model_name: "coding-route",
        litellm_params: { model: "codex-mini-latest" },
        model_info: { mode: "chat" },
      }),
  ...
  it("withholds catalog authority when the Vertex Anthropic adapter conflicts with the routed provider", () => {
  ...
  it("keeps baseline compatibility metadata for Responses-mode routes", async () => {
    // Compat is derived from the model id alone. If API-aware compat were
    // reintroduced at the call site, a Responses-mode Moonshot route would
  ...
  it.each([
    ...
    // Every untrusted string discovery reads. A YAML value such as
  ...
  it("withholds and diagnoses a route mixing chat and embedding deployments", async () => {
    const stderr = vi.spyOn(process.stderr, "write").mockReturnValue(true);
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [
            { model_name: "mixed-mode-route", model_info: { id: "chat", mode: "chat" } },
            { model_name: "mixed-mode-route", model_info: { id: "embed", mode: "embedding" } },
          ],
        }),
    });
    ...
    expect(String(stderr.mock.calls[0]?.[0])).toContain("mixed-mode-route");
...
describe("discoverModels wildcard expansion via /v1/models", () => {
  it("expands a wildcard /model/info entry and drops the literal wildcard id", async () => {
    ...
    expect(result.models.find((model) => model.id === "lemonade/Bonsai-1.7B-gguf")).toMatchObject({
      contextWindow: 128_000,
      api: "openai-completions",
    });
  ...
  it("inherits a wildcard parent's conservative context limit", async () => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [{ model_name: "team/*", model_info: { mode: "chat", max_input_tokens: 8_000 } }],
        }),
      "/v1/models": () => jsonResponse(200, { data: [{ id: "team/child" }] }),
    });

    const result = await discoverModels("https://litellm.example.com", "sk-test", {});

    expect(result.models).toEqual([expect.objectContaining({ id: "team/child", contextWindow: 8_000 })]);
  });

  it("inherits a wildcard parent's denied reasoning levels", async () => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [
  ...
  it("does not publish unrelated /v1/models ids during wildcard expansion", async () => {
    mockEndpoints({
      "/model/info": () => jsonResponse(200, { data: [{ model_name: "team/*", model_info: { mode: "chat" } }] }),
      "/v1/models": () => jsonResponse(200, { data: [{ id: "team/child" }, { id: "unrelated/model" }] }),
    });

    const result = await discoverModels("https://litellm.example.com", "sk-test", {});

    expect(result.models.map((model) => model.id)).toEqual(["team/child"]);
  });

  it("does not inherit suppression for a forced-thinking Moonshot child", async () => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [
            {
              model_name: "kimi/*",
              litellm_params: { model: "moonshot/kimi-k2" },
              model_info: { mode: "chat" },
            },
          ],
        }),
    ...
    expect(result.models[0]).toMatchObject({ id: "kimi/k2-thinking" });
    expect(result.models[0]).not.toHaveProperty("suppressReasoningContent");
  });

  it("suppresses children of a dropped wildcard route", async () => {
    mockEndpoints({
      "/model/info": () =>
        jsonResponse(200, {
          data: [
            { model_name: "team/*", model_info: { id: "chat", mode: "chat" } },
            { model_name: "team/*", model_info: { id: "embed", mode: "embedding" } },
            { model_name: "other/*", model_info: { mode: "chat" } },
          ],
        }),
      "/v1/models": () => jsonResponse(200, { data: [{ id: "team/child" }, { id: "other/child" }] }),
    });

    const result = await discoverModels("https://litellm.example.com", "sk-test", {});

    expect(result.models.map((model) => model.id)).toEqual(["other/child"]);
  });
...
describe("discoverModels response-mode models", () => {
  ...
  it("uses correlated detail model_name as the public health route", async () => {
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
      "/health": () =>
        jsonResponse(200, { healthy_endpoints: [{ model: "anthropic/claude-x", model_id: "claude-uuid" }] }),
      "/model/info?litellm_model_id=claude-uuid": () =>
        jsonResponse(200, {
          data: [
            {
    ...
    expect(result.models).toEqual([
      expect.objectContaining({ id: "team-claude", api: "openai-completions", contextWindow: 1_000_000 }),
    ]);
  });
  ...
  it("selects Responses for a health-only catalogued Responses route", async () => {
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
    ...
    expect(result.models[0]).toMatchObject({ id: "openai/gpt-5.5", api: "openai-responses" });
  });

  it("does not derive metadata from a health endpoint without deployment detail", async () => {
    // No `model_id`, so route text authorizes nothing but the transport supplied
    // by the Pi catalog. It cannot grant capabilities, limits, or pricing.
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
      "/health": () => jsonResponse(200, { healthy_endpoints: [{ model: "openai/gpt-5.5" }] }),
...
describe("discoverModels fallback to /health", () => {
  ...
  it("uses the correlated detail route and metadata when the health model differs", async () => {
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
  ...
  it("marks all health routes without deployment detail incomplete", async () => {
    // Route text authorizes nothing but transport for an evidence-free health
    // entry. It cannot grant capabilities, limits, pricing, or catalog authority.
    mockEndpoints({
      "/model/info": () => jsonResponse(404, {}),
      "/v1/models": () => jsonResponse(404, {}),
      "/health": () =>
