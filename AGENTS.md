...
- `closeSerializerPolicy` must gate every Chat or Responses level map, including catalog enrichment inside `enrichCachedModel`, whose compat stays as stored. Cached `anthropic-messages` models must bypass that OpenAI gate so their discovered Messages compatibility and level map round-trip unchanged.
