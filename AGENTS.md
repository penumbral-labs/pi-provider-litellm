...
- `api` values arriving from user `models.json` are unconstrained. Narrow with `isSelectableLiteLLMApi` before calling `resolveModelBaseUrl`: registry membership alone is not enough, because a protocol discovery cannot select is one this provider cannot govern end to end. `toNativeModels` is exempt because its input is discovery output, which is typed.
