...
- `litellmPolicy` carries both the request conclusion (`normalizeStrictToolMessages`) and the display conclusion (`normalizeThinkTags`). The `message_end` hook resolves the model through `ctx.modelRegistry` and reads that field; it must not re-derive a backend from the message's model id. Whether the route is Moonshot-backed is evidence-derived; the forced-thinking sub-decision inside `moonshotPolicy` is still a route-id pattern, so pass it the route id and never a semantic label — no label can match that pattern, which would silently make the exemption unreachable and split the conclusion by discovery source.
- `advertisableLevels` must gate every published level map, including the catalog enrichment inside `enrichCachedModel`, whose compat stays as stored.
