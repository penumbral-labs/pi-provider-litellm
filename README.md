...
- For `/model/info`, Responses is selected only when every row explicitly reports Responses mode; mixed Chat/Responses or unknown groups use Chat. A route that combines a chat-style deployment with an explicitly incompatible mode such as `embedding` is withheld entirely, because LiteLLM could otherwise select a deployment that cannot accept the request. Evidence-free `/health` and `/v1/models` entries use the Pi catalog's transport for the route: `openai-responses` when declared, and Chat otherwise. The route name authorizes no reasoning levels; `/health` entries without detail receive no other catalog metadata, while `/v1/models` entries may receive catalog name, reasoning, limits, and cost.
- Vision and reasoning are advertised only when every routable deployment resolves them as supported. A usable public reasoning-effort list is complete: omitted standard levels (`minimal`, `low`, `medium`, and `high`) are denied, while explicit LiteLLM `true` values add levels back and explicit `false` values remove them. A public list with no recognized values has no opinion, but `null` entries in a Pi-catalog level map remain explicit denials. Without a usable public list, standard levels with no LiteLLM or catalog-map opinion remain absent so Pi keeps its default behavior; `xhigh` and `max` always require explicit LiteLLM `true` from every deployment.
- Context and output limits use the smaller valid value when both router and catalog limits exist, then the minimum resolved value across deployments.
...
- In `/health` fallback, a correlated `/model/info` row's public `model_name` takes precedence because the health entry's `model` is its backend string; that health value is used only when detail is absent or does not name a route. Genuine per-endpoint `/model/info` rows receive the same public-catalog reasoning enrichment as aggregate rows, while synthetic health-only rows receive no metadata beyond catalog-derived transport.

The ` (no metadata)` suffix is reserved for evidence-free `/v1/models` fallback entries. Those entries may receive bounded catalog enrichment on a later cache read, including switching to `openai-responses` when the resolved Pi catalog entry declares that transport; all other catalog APIs remain Chat through LiteLLM. The ` (incomplete metadata)` suffix marks reduced `/model/info` groups or unresolved `/health` routes and permanently prevents route-name cache enrichment. It means at least one metadata field is unknown, including cache pricing that the proxy omitted; known input/output prices may still be shown alongside the suffix.

### Reasoning compatibility

Reasoning selectors require backend identity and a wire control accepted by every routable deployment; public route names do not authorize them. For `reasoning_effort`, a usable public catalog effort list is complete: omitted standard levels are denied, explicit LiteLLM `false` values remove levels, and explicit LiteLLM `true` values may add them back. A public list with no recognized values has no opinion, while `null` entries in a Pi-catalog level map remain denials. `xhigh` and `max` remain disabled unless every deployment explicitly reports `true`. When no usable public list exists, standard levels with no LiteLLM or catalog-map opinion remain absent so Pi keeps its defaults. The semantic generation contract owns whether `off` is possible when deployment evidence supplies an effort list, except that an explicit LiteLLM `supports_none_reasoning_effort: false` denial takes precedence: otherwise generations with a disable control keep their mapped `off`, while always-thinking generations keep `off: null`. A generation with a single on/off thinking switch (Kimi K2.5, K2.6, K2.7 Code) keeps its binary map when a host also accepts `reasoning_effort` and no usable public catalog states effort levels; explicit LiteLLM denials still close levels, and a usable public effort list governs the remaining standard levels.
...
Unknown generations and mixed deployment evidence expose no speculative selector. Kimi K2.7 Code, Kimi K3, and DeepSeek V4 retain assistant reasoning content for tool and multi-turn replay. Inline `<think>` normalization is a separate display policy: when any deployment declares backend evidence, it requires unanimous normal Kimi evidence; when none declares a backend, Kimi-shaped route-name evidence may enable response-only normalization, never generation controls or visibility suppression. Request-side reasoning visibility suppression additionally requires every deployment's LiteLLM routing provider to be Moonshot; `model_info.base_model` can identify Kimi for response normalization but never enables Moonshot-only request parameters on Azure, Bedrock, or another host. Strict Moonshot/Kimi tool-message repair is enabled only when every deployment identifies that family; otherwise discovery emits a bounded warning because tool calls may fail. Gemini effort case normalization likewise requires unanimous deployment-family evidence rather than a Gemini-looking route name. Existing GPT-5.5 Chat tool-request compatibility remains unchanged.
...
| `LiteLLM discovery: cached models predate discovery policy v2` | A legacy reasoning cache survived an offline or failed refresh. Safe response normalization is restored from stored compatibility evidence, but reasoning controls still require a successful network discovery to replace the stored list. Retry `/model` with network access. |
