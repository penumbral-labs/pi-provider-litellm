...
When deployments disagree about which backend serves a route, or when only some of them identify one, catalog limits, pricing, and reasoning metadata are withheld for the whole group. Disagreements where at least one deployment resolved a provider are additionally reported on stderr with a bounded list of affected route names. If only some deployments identify Moonshot/Kimi, strict tool-message repair is withheld and a warning names the affected routes. Cached models created before this policy existed also keep that outbound repair off until a successful refresh records unanimous deployment evidence.
