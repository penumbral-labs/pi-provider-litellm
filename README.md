...
- When a route's deployments supply missing or conflicting provider evidence — they name different providers, or one names a provider and another names nothing usable — catalog metadata is withheld for the whole group rather than guessed. A one-line diagnostic naming the affected routes is written to stderr once per process, so a persistent misconfiguration is announced rather than repeated on every background refresh.
