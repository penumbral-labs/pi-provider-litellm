...
The ` (no metadata)` suffix is reserved for evidence-free `/v1/models` fallback entries. A recognized `owned_by` provider that conflicts with a provider-qualified model ID withholds catalog enrichment rather than falling through to the ID prefix. Eligible entries may receive bounded catalog enrichment on a later cache read. The ` (incomplete metadata)` suffix marks reduced `/model/info` groups or unresolved `/health` routes and permanently prevents route-name cache enrichment. It means at least one metadata field is unknown, including cache pricing that the proxy omitted; known input/output prices may still be shown alongside the suffix.
