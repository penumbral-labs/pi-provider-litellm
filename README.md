...
- When complete base pricing and unanimous catalog authority are available, tiered pricing is the conservative worst-case envelope. Its thresholds are the sorted union of every deployment's thresholds, and each price field at each interval is the maximum applicable rate across all deployments, using a deployment's base rate until its first threshold. Differing tier breakpoints are therefore preserved rather than treated as incompatible.
- Tier ladders are intentionally omitted when complete base-pricing or catalog authority is unavailable. Wildcard expansion also omits tiers when any matching group has incomplete metadata, because partial ladders cannot provide a safe envelope; known base-price fields remain available.
