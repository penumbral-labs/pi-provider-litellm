...
A `/model/info` route group uses native Anthropic `/v1/messages` only when every deployment explicitly reports Chat mode, identifies a Claude backend through an Anthropic-, Bedrock-, or Vertex-capable adapter, and every declared backend identity resolves the same Anthropic compatibility policy. A `claude-<name>-<major>-<minor>` backend id resolves through its `claude-<name>-<major>` entry when the exact id is absent; an unknown generation stays on Chat Completions. When `supported_endpoints` is present, it must include `/v1/messages`; an explicit endpoint list without it keeps the group on Chat Completions. An unresolved routing or base-model identity denies native Messages rather than being discarded from the unanimity check. Mixed-generation, mixed-family, and unknown groups remain on Chat Completions; unanimous explicit Responses mode takes precedence. Routes discovered through `/health` never select native Messages: Messages candidates are downgraded to Chat Completions with reasoning controls closed, while correlated Responses detail remains on Responses. A health entry without detail takes only its transport from the Pi catalog. The `/v1/models` fallback takes its transport from the same catalog entry that supplies its name, reasoning capabilities and levels, input modalities, cost, and limits: `openai-responses` when the catalog says so, otherwise Chat Completions. Route text authorizes no capabilities, limits, prices, reasoning levels, or native Messages for a health-only entry. Catalog identity such as Amazon Bedrock remains separate from the selected wire protocol.

Native Messages authenticates with `x-api-key`; every transport carries the `x-litellm-session-id` header described above.
...
- Responses is selected only when every row explicitly reports Responses mode; mixed Chat/Responses or unknown groups use Chat. A route mixing a chat-style deployment with an explicitly incompatible mode such as embedding is withheld and reported once on stderr.
...
- Each deployment limit uses the smaller valid router and catalog value, then the route uses the minimum across deployments.
- Each displayed price field uses the maximum only when every deployment resolves that field; unresolved fields remain zero. Operator prices replace the same catalog field throughout tier ladders, and multi-deployment ladders use a conservative union-threshold envelope. A model name is suffixed with ` (incomplete metadata)` whenever any published capability, limit, or price remains unresolved.
- Wildcard routes expand only matching `/v1/models` ids. Expanded children inherit their surviving parents' conservative capabilities, limits, prices, and suppression policy; children of withheld wildcard routes remain hidden.
- Moonshot request-side reasoning suppression follows LiteLLM's routing provider, not `model_info.base_model`, so hosted Kimi deployments keep their transport's native reasoning behavior. `<think>` normalization on OpenAI-style transports follows that request flag as well as Moonshot-named model ids, including opaque route aliases whose Moonshot requests merge reasoning into response text; native Messages responses are left unchanged.
- Catalog metadata is accepted only from one unanimous concrete model identity derived from deployment fields such as `litellm_params.model`, `model_info.base_model`, and the LiteLLM adapter. Different concrete backends conflict even when they belong to the same provider or semantic family. Ambiguous groups are not matched across all Pi provider catalogs. Vertex and Bedrock adapters may use Anthropic catalog metadata for a `claude-…` backend id after their own provider catalog misses; other provider-qualified identities cannot override a recognized adapter. For example, a route spanning Anthropic and Bedrock Claude deployments keeps explicit router metadata but does not borrow either provider's catalog metadata.

The ` (no metadata)` suffix is reserved for evidence-free `/v1/models` fallback entries eligible for bounded catalog enrichment on a later cache read. That enrichment refreshes transport from the resolved catalog entry too: `openai-responses` when specified, otherwise Chat Completions, never native Messages. A recognized `owned_by` provider that conflicts with a provider-qualified model ID instead receives ` (incomplete metadata)`, permanently withholding cache enrichment rather than falling through to the ID prefix. The ` (incomplete metadata)` suffix marks reduced `/model/info` groups or unresolved `/health` routes and permanently prevents route-name cache enrichment. It means at least one metadata field is unknown, including cache pricing that the proxy omitted; known input/output prices may still be shown alongside the suffix.
...
| `LiteLLM discovery: ... route group(s) mix chat-style and explicitly incompatible deployment modes` | One public route mixes Chat/Responses deployments with an embedding or other non-chat mode. Split the deployments into separate route names; the mixed route is withheld because not every deployment can accept chat requests. |
