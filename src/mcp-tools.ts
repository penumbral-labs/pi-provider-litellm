...
const INVALID_ENTRY_IDENTITY_BUFFER_BYTES = 64 * 1024;
...
export interface McpDiscovery {
  ...
  partialFailure: boolean;
  // Entries dropped by normalization, with safe display labels and non-displayed identities.
  invalid: InvalidMcpTool[];
}
...
// Rejected entries have no registered name; positional labels keep stderr free of proxy text.
function invalidToolLabel(index: number): string {
  return `entry_${index}`;
}

// This structural encoding never retains or prints raw entry text. Length prefixes preserve
// boundaries, while the discovery body cap bounds the amount of input hashed. The per-process salt
// keeps low-entropy malformed entries from being recoverable by guessing.
...
function invalidToolIdentity(value: unknown): string {
  const digest = createHmac("sha256", invalidEntryIdentitySalt);
  const active = new WeakSet<object>();

  const writeText = (text: string): void => {
    let remaining = text;
    while (remaining.length > 0) {
      const { read, written } = invalidEntryIdentityEncoder.encodeInto(remaining, invalidEntryIdentityBuffer);
      if (written > 0) digest.update(invalidEntryIdentityBuffer.subarray(0, written));
      if (read === 0) throw new Error("Failed to encode MCP entry identity");
      remaining = remaining.slice(read);
    }
  };
  const writeString = (text: string): void => {
    writeText(`${byteLength(text)}:`);
    writeText(text);
  };
  type IdentityPart =
    | { kind: "value"; value: unknown }
    | { kind: "text" | "string"; value: string }
  ...
  while (pending.length > 0) {
    ...
    if (entry === null) {
      ...
      writeText(`number:${entry};`);
    } else if (typeof entry === "boolean") {
      writeText(entry ? "true;" : "false;");
    } else if (typeof entry !== "object") {
      writeText(`${typeof entry};`);
    } else if (active.has(entry)) {
      writeText("cycle;");
    } else if (Array.isArray(entry)) {
      active.add(entry);
      writeText(`array:${entry.length}[`);
      pending.push({ kind: "leave", value: entry }, { kind: "text", value: "]" });
      ...
      for (let index = fields.length - 1; index >= 0; index--) {
        const [key, child] = fields[index] as [string, unknown];
        pending.push({ kind: "value", value: child }, { kind: "string", value: key });
      }
    }
  }
...
export async function discoverMcpTools(
  ...
  try {
    ...
    rawTools.forEach((raw, index) => {
      ...
      else invalid.push({ label: invalidToolLabel(index), identity: invalidToolIdentity(raw) });
    });
...
export function prepareTools(discovery: McpDiscovery): {
  ...
  const dropMemberships = new Map<McpDropReason, string[]>();
  const degrades = new Map<McpDegradeReason, string[]>();
  const recordDrop = (reason: McpDropReason, label: string, identity = label): void => {
    const labels = drops.get(reason);
    if (labels) labels.push(label);
    else drops.set(reason, [label]);
    const identities = dropMemberships.get(reason);
    if (identities) identities.push(identity);
    else dropMemberships.set(reason, [identity]);
  };

  for (const invalid of discovery.invalid) {
    recordDrop("invalid-tool", invalid.label, `${invalid.label}\0${invalid.identity}`);
  }
  ...
  const report: McpPreparationReport = {
...
function emitPreparationDiagnostics(report: McpPreparationReport): void {
  ...
  for (const { reason, tools } of report.dropped) {
    const identities = dropMemberships.get(reason);
    if (!identities) throw new Error(`Missing MCP diagnostic membership for ${reason}`);
    emitClass(
      reason,
